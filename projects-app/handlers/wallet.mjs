...
export default async function handler(req, res) {
  ...
  const tma = extractInitData(req, body);
  if (!tma) return err(res, 400, 'missing_tma_or_admin_token', origin);

  if (!SUPABASE_URL || !SERVICE_ROLE || !BOT_TOKEN) return err(res, 500, 'server_misconfigured', origin);

  let tuser;
  try {
    tuser = verifyInitData(tma, BOT_TOKEN);
  } catch (error) {
    return err(res, 400, 'tma_invalid', origin);
  }
  if (!tuser?.ok) return err(res, 401, 'tma_invalid', origin);
  const userId = String(tuser.userId || tuser.user?.id || '');
  if (!userId) return err(res, 401, 'tma_invalid', origin);
